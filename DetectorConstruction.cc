...
G4VPhysicalVolume *MyDetectorConstruction::Construct() {
    ...
    // Main scintillator: 5×5×1 cm
    auto solidWorld = new G4Box("World", 0.5*m, 0.5*m, 0.5*m);
    logicWorld = new G4LogicalVolume(solidWorld, worldMat, "World");
    physWorld  = new G4PVPlacement(0, {}, logicWorld, "World", nullptr, false, 0, true);

    //Scintillator 5X5X1 cm
    auto solidScint = new G4Box("Scintillator", 2.5*cm, 2.5*cm, 0.5*cm);
    logicScint = new G4LogicalVolume(solidScint, scintMat, "Scintillator");

    new G4PVPlacement(0, {}, logicScint, "Scintillator", logicWorld, false, 0, true);

    // SIPM 6mm × 6mm × 1mm
    G4Material* Si = nist->FindOrBuildMaterial("G4_Si");

    auto solidSiPM = new G4Box("SiPM", 0.3*cm, 0.3*cm, 0.05*cm);
    logicSiPM = new G4LogicalVolume(solidSiPM, Si, "SiPM");

    G4double sipm_zpos = 1*cm + 0.05*cm;

    new G4PVPlacement(
        0,
        G4ThreeVector(0, 0, sipm_zpos),
        logicSiPM,
        "SiPM",
        logicWorld,
        false,
    ...
    // Scintillatino Bar Design with fibers :::::::::
    // change dimensions here 
    //G4Box *solidScint = new G4Box("solidScint", 5*cm, 5*cm, 1*cm); 
    // Define Logical Volume
